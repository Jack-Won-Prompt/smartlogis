...
<head>
    ...
    <title>{{ $barTitle }} · SmartLogis</title>
    <style>
        *{ box-sizing:border-box; }
        body{ margin:0; background:#f1f5f9; font-family:'맑은 고딕','Malgun Gothic',sans-serif; color:#1e293b; }
        .bar{ position:sticky; top:0; z-index:5; display:flex; align-items:center; justify-content:space-between; gap:12px; padding:12px 20px; background:#0b1a33; color:#fff; }
        ...
        @media print{ .bar{ display:none; } body{ background:#fff; } .sheet{ margin:0; border:none; border-radius:0; page-break-after:always; } .sheet:last-child{ page-break-after:auto; } }
    </style>
</head>
<body>
    <div class="bar">
    ...
    <div class="sheet">
        ...
        <div class="route">
            <div class="box"><div class="lbl">{{ $s['fromLabel'] }}</div><div class="val">{{ $s['fromName'] }}</div></div>
            <div class="arrow">→</div>
            <div class="box"><div class="lbl">{{ $s['toLabel'] }}</div><div class="val">{{ $s['toName'] }}</div></div>
        </div>
        ...
        <table class="items">
            ...
            <tfoot>
                <tr><td colspan="5" class="r">합계 수량</td><td class="r">{{ number_format(collect($s['items'])->sum('qty')) }}</td></tr>
            </tfoot>
        </table>

        <div class="sign">
            <div class="slot">{{ $s['signLeft'] }}<div class="line"></div></div>
            <div class="slot">{{ $s['signRight'] }}<div class="line"></div></div>
        </div>

        <div class="foot">SmartLogis · 출력일 {{ now()->timezone('Asia/Seoul')->format('Y-m-d H:i') }} · 본 지시서의 QR 은 문서번호({{ $s['docNo'] }})를 담고 있습니다.</div>
    </div>
    @empty
        <div class="sheet"><p style="text-align:center;color:#94a3b8;padding:40px">출력할 지시서가 없습니다.</p></div>
    @endforelse
</body>
</html>
